...
BeforeAll {
    ...
    function ThenUseApprovedVerbs
    {
        ...
        $verbs =
            Get-Command -Module 'Carbon.Accounts'|
...
Describe 'Carbon.Accounts' {
    It 'should have about help topic' {
        GivenModuleImported
        ThenHelpTopic 'about_Carbon.Accounts' -Exists
    }
    ...
    It 'should have a help topic for each command' {
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'Carbon.Accounts' -CommandType Function,Cmdlet,Filter))
        {
